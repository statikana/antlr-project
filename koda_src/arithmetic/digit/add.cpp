   ...
   public:
	...
	Instance operator()(const BoolT lhs) {
		if (holds<BoolT>(rhs.data)) {
			return Instance::new_int(
				lhs + static_cast<IntT>(get<BoolT>(rhs.data)));
			...
			return Instance::new_float(
				static_cast<FloatT>(lhs) + get<FloatT>(rhs.data));
		}
		throw "unexpected operation";
	}
	...
	Instance operator()(const FloatT lhs) {
		if (holds<std::monostate>(rhs.data)) {
			...
			return Instance::new_float(
				lhs + static_cast<FloatT>(get<BoolT>(rhs.data)));
		} else if (holds<IntT>(rhs.data)) {
			...
			return Instance::new_string(
				std::to_string(lhs) + get<StringT>(rhs.data));
		}
		throw "unexpected operation";
	}
	...
	Instance operator()(const ArrayData lhs) {
		ArrayData new_arr;
