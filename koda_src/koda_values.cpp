...
template <typename T>
bool holds(const Data &var) {
	return std::holds_alternative<T>(var);
}

template <typename T>
T get(const Data &var) {
...
class Instance {
   public:
	...
	static Instance new_int(long int value) {
		...
		switch (type->get_kind()) {
			...
			default:
				throw std::runtime_error(
					"to_text undefined for type " + type->get_name());
		}
	}
};