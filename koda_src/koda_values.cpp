...
struct StructData {
	public:
	StructData(std::map<std::string, std::shared_ptr<Instance>> fields) { this->fields = std::move(fields); }
...
// any Data type using Instance must use a shared pointer to it, so the size
// is known to the compiler before initializing Instance class.
...
template<typename T> bool holds(const Data& var) { return std::holds_alternative<T>(var); }
...
class Instance {
	...
	static Instance new_bool(BoolT value) { return Instance(value, g_BoolType); }

	static Instance new_int(IntT value) { return Instance(value, g_IntType); }

	static Instance new_float(FloatT value) { return Instance(value, g_FloatType); }

	static Instance new_string(StringT value) { return Instance(value, g_StringType); }

	static Instance new_array(ArrayData value) { return Instance(value, g_ArrayType); }

	static Instance new_struct(StructData value, std::shared_ptr<Type> s_type) { return Instance(value, s_type); }

	Instance clone_shallow() {
		return Instance(
			data, type
		);
	}
	...
	std::string to_text() const {
		...
		case ARRAY: {
			std::string str = "[";
			ArrayData raw	= get<ArrayData>(data);
			if (raw.size() == 0) {
				str.append("]");
				return str;
			}
