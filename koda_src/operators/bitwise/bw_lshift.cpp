...
class BwLshiftVisitor : public VisitorBase {
  public:
    const Instance& rhs;
    BwLshiftVisitor(const Instance& other) : VisitorBase(other), rhs(other) {}

    Instance operator()(const VoidT lhs) { throw MSG_BAD_CALL; }

    Instance operator()(const BoolT lhs) {
        if (holds<BoolT>(rhs.data)) {
            return Instance::new_int(static_cast<int>(lhs) << static_cast<int>(get<BoolT>(rhs.data)));
        }
        throw MSG_BAD_CALL;
    }
