...
AtomV _div(const AtomVAccess& la, const AtomVAccess& ra) {
	// assumes RHS is a subtype of LHS (will get called twice)
	if (la.get_kind() != TYPE_KIND or ra.get_kind() != TYPE_KIND) {
		throw std::runtime_error("div not supported for non-type-kinds");
	}
...
AtomV div(const AtomV& lhs, const AtomV& rhs) {
	...
	try {
		return _div(la, ra);
	} catch (NotImplementedT) { return _div(ra, la); }
}