...
AtomV _sub(const AtomVAccess& la, const AtomVAccess& ra) {
	// assumes RHS is a subtype of LHS (will get called twice)
	if (la.get_kind() != TYPE_KIND or ra.get_kind() != TYPE_KIND) {
		throw std::runtime_error("sub not supported for non-type-kinds");
	}
	...
	case STRING_TYPE: {
		if (!type_subtype(STRING_TYPE, ra.get_type())) {
			throw NotImplementedT();
		}
		string lval = get<shared_ptr<String>>(la.inner)->get_value();
		string rval = get<shared_ptr<String>>(ra.inner)->get_value();
		if (rval.size() > lval.size() or lval.substr(lval.size() - rval.size()) != rval) {
			throw NotImplementedT();
		}
		string result = lval.substr(0, lval.size() - rval.size());
...
AtomV sub(const AtomV& lhs, const AtomV& rhs) {
	...
	try {
		return _sub(la, ra);
	} catch (NotImplementedT) { return _sub(ra, la); }
}